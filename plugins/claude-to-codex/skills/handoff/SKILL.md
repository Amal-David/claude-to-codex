...
`--check`, or `--codex-subagents`:

```bash
node plugins/claude-to-codex/scripts/claude-to-codex.mjs --mode print
```

Token rule: pass Codex hot context, handoff manifest, git snapshot, digest, and transcript paths, not a full pasted transcript. Let Codex read only the transcript slices it needs.
